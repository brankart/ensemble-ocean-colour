...
while [ $member -le ${upd_size} ] ; do
  ...
  if [ ${fdiag1} = 'time' ] ; then
    ncks -d x,$idiag,$idiag -d y,$jdiag,$jdiag -d deptht,$kdiag,$kdiag \
         -v $vdiag1 ${updir}/vct_${fdiag1}_${membertag4}.nc tmp_series.nc
  else
    if [ ${fdiag1} = 'diag' ] ; then
  ...
  if [ ${fdiag1} = 'time' ] ; then
    ncks -d x,$idiag,$idiag -d y,$jdiag,$jdiag -d deptht,$kdiag,$kdiag \
         -v $vdiag2 ${updir}/vct_${fdiag2}_${membertag4}.nc tmp_series.nc
  else
    if [ ${fdiag2} = 'diag' ] ; then
      ncks -d x,$idiag,$idiag -d y,$jdiag,$jdiag -d time_counter,$tdiag,$tdiag \
	   -v $vdiag2 ${updir}/vct_${fdiag2}_${membertag4}.nc tmp_series.nc
...
paste tmp_scatterplot_1.txt tmp_scatterplot_2.txt | tr ';' ' ' > ${figdir}/${exptdiag}_${vdiag1}_${vdiag2}_i${idiag}j${jdiag}t${tdiag}.txt
rm -f tmp_scatterplot_1.txt tmp_scatterplot_2.txt
